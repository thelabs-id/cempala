...
export function checkTask(db: DB, input: CheckTaskInput): Result<CheckTaskOutput> {
  ...
  if (row.status === "running") {
    ...
    if (liveness.state !== "running") {
      ...
      // read, and the second also means the run did not really succeed.
      // The rule itself lives in agent-output.ts so this path, dispatch's
      // three and the reaper cannot drift apart again.
      const settled = reconcileOutcome({
        resultText,
        exitCode,
        readStderr: () => {
      ...
      const finalStatus = settled.ok ? "completed" : "failed";
      const finalResult = settled.resultText;
      // `settled.exitCode`, not the code we started with. When stderr
      // overturns an apparent success the two differ, and persisting the
      // original would store `failed` beside `exit_code: 0` — a row that
      // contradicts itself and a caller that cannot tell which half to
      // believe.
      const finalExitCode = settled.exitCode;
      const now = Date.now();
      db.run(
        `UPDATE tasks SET status = ?, exit_code = ?, result = ?, completed_at = ? WHERE id = ? AND status = 'running'`,
        [finalStatus, finalExitCode, finalResult, now, input.task_id],
      );
      return {
        ok: true,
        data: { ...row, status: finalStatus, exit_code: finalExitCode, result: finalResult, completed_at: now },
      };
    }
  }
